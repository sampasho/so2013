...
main() {
   ...
   while(!feof(fp)){
		...
		sono_il_padre = fork();
		if (sono_il_padre == 0){
			printf("\n eksekutome cooperante: %s %s %s %s %s.\n" , time_upload,  upload_val, upload_live, time_to_live,  pari_dispari_pid_download, quantita_download);
			...
			fscanf(fp, "%s %s %s %s %s", time_upload, upload_val, upload_live, time_to_live, pari_dispari_pid_download, quantita_download);
		}
	}
